        ...
        <div class="p-6">
            ...
            <form action="{{ route('admin.interns.update', $intern) }}" method="POST" class="space-y-6">
                ...
                <div class="space-y-2">
                    ...
                    <select class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm @error('role_id') border-red-500 @enderror" id="role" name="role_id" required>
                        <option value="">Select Role</option>
                        @foreach($roles as $role)
                            <option value="{{ $role->id }}" {{ (old('role_id', $intern->role_id) == $role->id) ? 'selected' : '' }}>
                                {{ ucfirst($role->name) }}
                            </option>
                        @endforeach
                    </select>
